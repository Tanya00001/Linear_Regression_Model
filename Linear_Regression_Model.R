...
plot(salary1)
    #OR
plot(salary1$salary,salary1$year_of_experience) #Through $ sign we can access specific column in a dataset.

#Linear regression....
#Equation y=mx+c where y works as response variable, x works as input variable, m is slope and c is the intercept.

lm(salary1$salary ~ salary1$year_of_experience) #Through this you will get intercept and slope.
#Here lm is linear model and ~ specifies that salary regress on year_of_experience.
